...
class SKMultiRangeTest : public benchmark::Fixture {
public:
    void SetUp(const ::benchmark::State& state) override {
        ...
        set = std::make_unique<SkipList<uint64_t, std::less<>>>();
        for (int i = 0; i < size; ++i) {
            set->Insert(data[i]);
        }
...
BENCHMARK_DEFINE_F(SKMultiRangeTest, RangeQuery)(benchmark::State& state) {
...
BENCHMARK_REGISTER_F(SKMultiRangeTest, RangeQuery)
